...
def next_move(input, position, wire):
    direction, value = parse_move(input)
    if value <= 0:
        print(f"bad value {value}")

    if direction == 'R':
        p = right(value, position, wire)
    elif direction == 'L':
        p = left(value, position, wire)
    elif direction == 'U':
        p = up(value, position, wire)
    elif direction == 'D':
        p = down(value, position, wire)
    else:
        print(f"Bad input {input} and {position}")
...
def move(position, wire):
    ...
    dist += 1
    if position in MAZE:
        if MAZE[position] != wire:
            if wire == 2 and MAZE[position] == 1:
                DIST_CROSS.append(DIST[position] + dist)
            MAZE[position] = 'X'
            CROSS[position] = abs(position[0]) + abs(position[1])
    else:
        MAZE[position] = wire
        DIST[position] = dist
...
p = (0, 0)
for text in INPUT1:
    p = next_move(text, p, 1)

p = (0, 0)
dist = 0
for text in INPUT2:
    p = next_move(text, p, 2)

# print(CROSS)
d = sorted(CROSS, key=CROSS.get)[0]
print(abs(d[0])+abs(d[1]))
print(sorted(DIST_CROSS))
