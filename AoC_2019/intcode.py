class IntCode:
    ...
    def run(self, debug=False, inputs=None, print_outputs=False, halt_on_output=False,
        ...
        while True:
            ...
            if opcode == 1:
                value = self.get_value(self.pointer + 1, modes[0]) + self.get_value(
                ...
                value = 1 if self.get_value(self.pointer + 1, modes[0]) < self.get_value(
                    self.pointer + 2, modes[1]) else 0
                self.set_value(self.pointer + 3, modes[2], value)
                self.pointer += 4
            elif opcode == 8:
                value = 1 if self.get_value(self.pointer + 1, modes[0]) == self.get_value(
